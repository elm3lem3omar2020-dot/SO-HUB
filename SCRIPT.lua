-- الحماية من تعطل السكريبت إذا كان الرابط الخارجي معطلاً أو به أخطاء
pcall(function()
    loadstring(game:HttpGet("https://raw.githubusercontent.com/elm3lem3omar2020-dot/SO-HUB/refs/heads/main/FASTATTACK.lua"))()
end)

-- تعريفات مسبقة لتجنب خطأ (attempt to call a nil value)
local _tp, notween, BringEnemy, EquipWeapon, weaponSc, Useskills
local statsSetings, GetConnectionEnemies, LowCpu, CheckF, CheckBoat
local CheckEnemiesBoat, CheckPirateGrandBrigade, CheckShark, CheckTerrorShark
local CheckPiranha, CheckFishCrew, CheckHauntedCrew, CheckSeaBeast
local CheckLeviathan, UpdStFruit, collectFruits, Getmoon, DropFruits
local GetBP, GetIn, GetM, GetWP, getInfinity_Ability, Hop, BTP, TeleportToTarget
local Marines, Pirates
local PosMon = Vector3.new(0, 0, 0)

-- بدائل في حال كان المحاكي لا يدعم هذه الدوال
...
local R = d.Character and d.Character:FindFirstChild("HumanoidRootPart") or d.CharacterAdded:Wait():WaitForChild("HumanoidRootPart");
...
local D = d.Character:WaitForChild("Energy").Value;
...
repeat
	local loadCheck = (d.PlayerGui:WaitForChild("Main")):WaitForChild("Loading") and game:IsLoaded();
	task.wait();
until loadCheck;
...
local G = {
	"Snow Lurker", "Arctic Warrior", "Hidden Key", "Awakened Ice Admiral"
};
local q = {
	Mob = "Mythological Pirate", Mob2 = "Cursed Skeleton",
	"Hell\'s Messenger", Mob3 = "Cursed Skeleton", "Heaven\'s Guardian"
};
local t = {
	"Part", "SpawnLocation", "Terrain", "WedgePart", "MeshPart"
};
local X = { "Swan Pirate", "Jeremy" };
local h = { "Forest Pirate", "Captain Elephant" };
local B = { "Fajita", "Jeremy", "Diamond" };
local l = {
	"Beast Hunter", "Lantern", "Guardian", "Grand Brigade",
	"Dinghy", "Sloop", "The Sentinel"
};
...
local gay = (function()
	...
	if d and game:GetService("Lighting") then
		local Y = d:FindFirstChild("DarkFog");
		if Y then Y:Destroy(); end;
	end;
	local R = workspace._WorldOrigin:FindFirstChild("Foam;");
...
f.Pos = function(Y, d)
    -- تم إصلاح mode إلى Y.HumanoidRootPart لتجنب أخطاء إضافية
...
f.Kill = function(Y, dArg)
	if Y and dArg then
		...
		local curTool = game.Players.LocalPlayer.Character:FindFirstChildOfClass("Tool");
		local R = curTool and curTool.ToolTip or "";
...
statsSetings = function(Y, R)
	if Y == "Melee" then
		if d.Data.Points.Value ~= 0 then Q.Remotes.CommF_:InvokeServer("AddPoint", "Melee", R); end;
	elseif Y == "Defense" then
		if d.Data.Points.Value ~= 0 then Q.Remotes.CommF_:InvokeServer("AddPoint", "Defense", R); end;
	elseif Y == "Sword" then
		if d.Data.Points.Value ~= 0 then Q.Remotes.CommF_:InvokeServer("AddPoint", "Sword", R); end;
	elseif Y == "Gun" then
		if d.Data.Points.Value ~= 0 then Q.Remotes.CommF_:InvokeServer("AddPoint", "Gun", R); end;
	elseif Y == "Devil" then
		if d.Data.Points.Value ~= 0 then Q.Remotes.CommF_:InvokeServer("AddPoint", "Demon Fruit", R); end;
	end;
end;
...
BringEnemy = function()
	...
	for _, R in pairs(workspace.Enemies:GetChildren()) do
		if R:FindFirstChild("Humanoid") and R.Humanoid.Health > 0 then
			if R.PrimaryPart and (R.PrimaryPart.Position - PosMon).Magnitude <= 300 then
...
Useskills = function(Y, dArg)
	if Y == "Melee" then
		weaponSc("Melee");
		if dArg == "Z" or dArg == "X" or dArg == "C" then
			K:SendKeyEvent(true, dArg, false, game);
			K:SendKeyEvent(false, dArg, false, game);
		end;
	elseif Y == "Sword" then
		weaponSc("Sword");
		if dArg == "Z" or dArg == "X" then
			K:SendKeyEvent(true, dArg, false, game);
			K:SendKeyEvent(false, dArg, false, game);
		end;
	elseif Y == "Blox Fruit" then
		weaponSc("Blox Fruit");
		if dArg == "Z" or dArg == "X" or dArg == "C" or dArg == "V" then
			K:SendKeyEvent(true, dArg, false, game);
			K:SendKeyEvent(false, dArg, false, game);
		end;
	elseif Y == "Gun" then
...
s.__namecall = newcclosure(function(...)
	...
	if tostring(Y) == "FireServer" then
		if tostring(dArg[1]) == "RemoteEvent" then
			if tostring(dArg[2]) ~= "true" and tostring(dArg[2]) ~= "false" then
				if _G.FarmMastery_G and not b or _G.FarmMastery_Dev or _G.FarmBlazeEM or _G.Prehis_Skills or _G.SeaBeast1 or _G.FishBoat or _G.PGB or _G.Leviathan1 or _G.Complete_Trials or (_G.AimMethod and ABmethod == "AimBots Skill") or (_G.AimMethod and ABmethod == "Auto Aimbots") then
					if MousePos then dArg[2] = MousePos; end
					return x(unpack(dArg));
				end;
			end;
		end;
	end;
...
GetConnectionEnemies = function(Y)
	...
	for _, R in next, game.Workspace.Enemies:GetChildren() do
		if R:IsA("Model") and ((typeof(Y) == "table" and table.find(Y, R.Name) or R.Name == Y) and (R:FindFirstChild("Humanoid") and R.Humanoid.Health > 0)) then
			return R;
		end;
	end;
...
LowCpu = function()
	local Y = true;
	local curD = game;
	local R = curD.Workspace;
	local curQ = curD.Lighting;
	local curR = R.Terrain;
	curR.WaterWaveSize = 0;
	curR.WaterWaveSpeed = 0;
	curR.WaterReflectance = 0;
	curR.WaterTransparency = 0;
	curQ.GlobalShadows = false;
	curQ.FogEnd = 9000000000.0;
	curQ.Brightness = 0;
	(settings()).Rendering.QualityLevel = "Level01";
	for _, R in pairs(curD:GetDescendants()) do
		...
		elseif R:IsA("Explosion") then
			R.BlastPressure = 1; R.BlastRadius = 1;
		elseif R:IsA("Fire") or R:IsA("SpotLight") or R:IsA("Smoke") or R:IsA("Sparkles") then
			R.Enabled = false;
		elseif R:IsA("MeshPart") then
			R.Material = "Plastic"; R.Reflectance = 0; R.TextureID = 10385902758728957;
		end;
	end;
	for _, curD2 in pairs(curQ:GetChildren()) do
		if curD2:IsA("BlurEffect") or curD2:IsA("SunRaysEffect") or curD2:IsA("ColorCorrectionEffect") or curD2:IsA("BloomEffect") or curD2:IsA("DepthOfFieldEffect") then
			curD2.Enabled = false;
		end;
	end;
end;
...
CheckShark = function()
	for _, R in pairs(workspace.Enemies:GetChildren()) do
		if R.Name == "Shark" and f.Alive(R) then return true; end;
	end;
	return false;
end;
...
CheckTerrorShark = function()
	for _, R in pairs(workspace.Enemies:GetChildren()) do
		if R.Name == "Terrorshark" and f.Alive(R) then return true; end;
	end;
	return false;
end;
...
CheckPiranha = function()
	for _, R in pairs(workspace.Enemies:GetChildren()) do
		if R.Name == "Piranha" and f.Alive(R) then return true; end;
	end;
	return false;
end;
...
CheckFishCrew = function()
	for _, R in pairs(workspace.Enemies:GetChildren()) do
		if (R.Name == "Fish Crew Member" or R.Name == "Haunted Crew Member") and f.Alive(R) then return true; end;
	end;
	return false;
end;
...
CheckHauntedCrew = function()
	for _, R in pairs(workspace.Enemies:GetChildren()) do
		if R.Name == "Haunted Crew Member" and f.Alive(R) then return true; end;
	end;
	return false;
end;
...
UpdStFruit = function()
	for _, R in next, d.Backpack:GetChildren() do
		local StoreFruit = R:FindFirstChild("EatRemote", true);
		if StoreFruit then
...
DropFruits = function()
	...
	for _, R in pairs(d.Character:GetChildren()) do
		if string.find(R.Name, "Fruit") then
			EquipWeapon(R.Name); wait(.1);
			if d.PlayerGui.Main.Dialogue.Visible == true then
				d.PlayerGui.Main.Dialogue.Visible = false;
...
GetIn = function(Y)
	for _, QItem in pairs(Q.Remotes.CommF_:InvokeServer("getInventory")) do
		if type(QItem) == "table" then
			if QItem.Name == Y or d.Character:FindFirstChild(Y) or d.Backpack:FindFirstChild(Y) then
				return true;
			end;
		end;
	end;
...
GetM = function(Y)
	for _, RItem in pairs(Q.Remotes.CommF_:InvokeServer("getInventory")) do
		if type(RItem) == "table" then
			if RItem.Type == "Material" and RItem.Name == Y then
				return RItem.Count;
			end;
		end;
	end;
...
GetWP = function(Y)
	for _, QItem in pairs(Q.Remotes.CommF_:InvokeServer("getInventory")) do
		if type(QItem) == "table" then
			if QItem.Type == "Sword" and (QItem.Name == Y or d.Character:FindFirstChild(Y) or d.Backpack:FindFirstChild(Y)) then
				return true;
			end;
		end;
	end;
...
getInfinity_Ability = function(Y, QArg)
	if not d.Character or not d.Character:FindFirstChild("HumanoidRootPart") then return; end;
	if Y == "Soru" and QArg then
		for _, RItem in next, getgc() do
			if d.Character:FindFirstChild("Soru") then
				if typeof(RItem) == "function" and (getfenv(RItem)).script == d.Character.Soru then
					for _, uVal in next, getupvalues(RItem) do
						if typeof(uVal) == "table" then
							task.spawn(function()
								repeat
									wait(T);
									uVal.LastUse = 0;
								until not QArg or d.Character.Humanoid.Health <= 0;
							end)
						end;
					end;
				end;
	...
	elseif Y == "Energy" and QArg then
		d.Character.Energy.Changed:connect(function()
			if QArg then d.Character.Energy.Value = D; end;
		end);
	elseif Y == "Observation" and QArg then
		local YRadius = d:FindFirstChild("VisionRadius");
		if YRadius then YRadius.Value = math.huge; end
	end;
end;
...
Hop = function()
	pcall(function()
		for i = math.random(1, math.random(40, 75)), 100, 1 do
			local Hops = Q.__ServerBrowser:InvokeServer(i);
			for place, info in next, Hops do
				if tonumber(info.Count) < 12 then
					a:TeleportToPlaceInstance(game.PlaceId, place);
				end;
			end;
		end;
	end);
...
task.spawn(function()
	...
	repeat task.wait(); until ply.Character and ply.Character.PrimaryPart;
	J.CFrame = ply.Character.PrimaryPart.CFrame;
	while task.wait() do
		pcall(function()
			if (getgenv()).OnFarm then
				if J and J.Parent == workspace then
					local curD = ply.Character and ply.Character.PrimaryPart;
					if curD and (curD.Position - J.Position).Magnitude <= 200 then
						curD.CFrame = J.CFrame;
					else
						J.CFrame = curD.CFrame;
					end;
				end;
...
_tp = function(Y)
	...
	if not char or char:FindFirstChild("HumanoidRootPart") == nil then return; end;
	local QPart = char.HumanoidRootPart;
	local dist = (Y.Position - QPart.Position).Magnitude;
	local aInfo = TweenInfo.new(dist / 300, Enum.EasingStyle.Linear);
	local wTween = (game:GetService("TweenService")):Create(J, aInfo, { CFrame = Y });
	if char.Humanoid.Sit == true then
		J.CFrame = CFrame.new(J.Position.X, Y.Y, J.Position.Z);
	end;
	wTween:Play();
	task.spawn(function()
		while wTween.PlaybackState == Enum.PlaybackState.Playing do
			if not y then wTween:Cancel(); break; end;
			task.wait(.1);
		end;
	end);
end;
...
notween = function(Y)
    if d.Character and d.Character:FindFirstChild("HumanoidRootPart") then
...
BTP = function(Y)
	...
	local QHum = char:FindFirstChild("Humanoid");
	local rGui = d.PlayerGui.Main;
	local startPos = RPart.Position;
	repeat
		QHum.Health = 0;
		RPart.CFrame = Y;
		rGui.Quest.Visible = false;
		if (RPart.Position - startPos).Magnitude > 1 then
			startPos = RPart.Position;
			RPart.CFrame = Y;
		end;
		task.wait(.5);
	until (Y.Position - RPart.Position).Magnitude <= 2000;
end;
		...
		pcall(function()
			if _G.SailBoat_Hydra or _G.WardenBoss or _G.AutoFactory or _G.HighestMirage or _G.HCM or _G.PGB or _G.Leviathan1 or _G.UPGDrago or _G.Complete_Trials or _G.TpDrago_Prehis or _G.BuyDrago or _G.AutoFireFlowers or _G.DT_Uzoth or _G.AutoBerry or _G.Prehis_Find or _G.Prehis_Skills or _G.Prehis_DB or _G.Prehis_DE or _G.FarmBlazeEM or _G.Dojoo or _G.CollectPresent or _G.AutoLawKak or _G.TpLab or _G.AutoPhoenixF or _G.AutoFarmChest or _G.AutoHytHallow or _G.LongsWord or _G.BlackSpikey or _G.AutoHolyTorch or _G.TrainDrago or _G.AutoSaber or _G.FarmMastery_Dev or _G.CitizenQuest or _G.AutoEctoplasm or _G.KeysRen or _G.Auto_Rainbow_Haki or _G.obsFarm or _G.AutoBigmom or _G.Doughv2 or _G.AuraBoss or _G.Raiding or _G.Auto_Cavender or _G.TpPly or _G.Bartilo_Quest or _G.Level or _G.FarmEliteHunt or _G.AutoZou or _G.AutoFarm_Bone or (getgenv()).AutoMaterial or _G.CraftVM or _G.FrozenTP or _G.TPDoor or _G.AcientOne or _G.AutoFarmNear or _G.AutoRaidCastle or _G.DarkBladev3 or _G.AutoFarmRaid or _G.Auto_Cake_Prince or _G.Addealer or _G.TPNpc or _G.TwinHook or _G.FindMirage or _G.FarmChestM or _G.Shark or _G.TerrorShark or _G.Piranha or _G.MobCrew or _G.SeaBeast1 or _G.FishBoat or _G.AutoPole or _G.AutoPoleV2 or _G.Auto_SuperHuman or _G.AutoDeathStep or _G.Auto_SharkMan_Karate or _G.Auto_Electric_Claw or _G.AutoDragonTalon or _G.Auto_Def_DarkCoat or _G.Auto_God_Human or _G.Auto_Tushita or _G.AutoMatSoul or _G.AutoKenVTWO or _G.AutoSerpentBow or _G.AutoFMon or _G.Auto_Soul_Guitar or _G.TPGEAR or _G.AutoSaw or _G.AutoTridentW2 or _G.Auto_StartRaid or _G.AutoEvoRace or _G.AutoGetQuestBounty or _G.MarinesCoat or _G.TravelDres or _G.Defeating or _G.DummyMan or _G.Auto_Yama or _G.Auto_SwanGG or _G.SwanCoat or _G.AutoEcBoss or _G.Auto_Mink or _G.Auto_Human or _G.Auto_Skypiea or _G.Auto_Fish or _G.CDK_TS or _G.CDK_YM or _G.CDK or _G.AutoFarmGodChalice or _G.AutoFistDarkness or _G.AutoMiror or _G.Teleport or _G.AutoKilo or _G.AutoGetUsoap or _G.Praying or _G.TryLucky or _G.AutoColShad or _G.AutoUnHaki or _G.Auto_DonAcces or _G.AutoRipIngay or _G.DragoV3 or _G.DragoV1 or _G.SailBoats or NextIs or _G.FarmGodChalice or _G.IceBossRen or senth or senth2 or _G.Lvthan or _G.beasthunter or _G.DangerLV or _G.Relic123 or _G.tweenKitsune or _G.Collect_Ember or _G.AutofindKitIs or _G.snaguine or _G.TwFruits or _G.tweenKitShrine or _G.Tp_LgS or _G.Tp_MasterA or _G.tweenShrine or _G.FarmMastery_G or _G.FarmMastery_S then
				...
				if not d.Character.HumanoidRootPart:FindFirstChild("BodyClip") then
					local bClip = Instance.new("BodyVelocity");
					bClip.Name = "BodyClip";
					bClip.Parent = d.Character.HumanoidRootPart;
					bClip.MaxForce = Vector3.new(100000, 100000, 100000);
					bClip.Velocity = Vector3.new(0, 0, 0);
				end;
				if not d.Character:FindFirstChild("highlight") then
					local hLight = Instance.new("Highlight");
					hLight.Name = "highlight";
					hLight.Enabled = true;
					hLight.FillColor = Color3.fromRGB(0, 255, 255);
					hLight.OutlineColor = Color3.fromRGB(255, 255, 255);
					hLight.FillTransparency = .5;
					hLight.OutlineTransparency = .2;
					hLight.Parent = d.Character;
				end;
				for _, part in pairs(d.Character:GetDescendants()) do
					if part:IsA("BasePart") then part.CanCollide = false; end;
				end;
			else
				y = false;
